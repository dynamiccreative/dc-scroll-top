 ...
 * @package RYSE\DCScrollTop
 * @version 1.1.0
 */
class DstGitHubUpdater
{
    /**
    ...
    {
        $this->pluginIcon = $file;//ltrim($file, '/');
    ...
    {
        $this->pluginBannerSmall = $file;//ltrim($file, '/');
    ...
    {
        $this->pluginBannerLarge = $file;//ltrim($file, '/');
    ...
    ): array|false|object
    {
        ...
        $result['last_updated'] = gmdate( 'Y-m-d' );
        $result['sections']['description'] = $fields['Description'];
    ...
    private function getChangelog(): string
    {
        ...
        if (!$this->changelog) {
            return 'See the full changelog on <a href="' . esc_url( $this->gitHubUrl ) . '/releases">GitHub</a>.';
            //return '';
        }
